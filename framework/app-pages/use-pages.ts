...
import { app_pages, roles } from "@/lib/prisma/generated/client";
...
// Implementation
export function usePages(params?: appPageQueryType) {
  let queryKey = ["app", "pages"];
  if (params?.cur_users) {
  ...
  return useQuery({
    queryKey,
    queryFn: async () => {
      const { data } = await http.get("/app-pages", { params });

      if (params?.only_ids === true) {
        return data as number[];
      }

      return data as app_pages[];
    },
  });
}
